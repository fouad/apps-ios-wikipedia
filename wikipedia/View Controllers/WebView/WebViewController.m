...
NSString *articleImageName = fetchedData[@"mobileview"][@"image"][@"file"];

//TODO: i made the ArticleFetcher temporarily return the dictionary it retreived so i could
// access articleImageName. But i think once article.image is in place we'll get it from there.
				...
				[self displayArticle:article.title leadImageName:articleImageName];
                    ...
                    discoveryMethod: (MWKHistoryDiscoveryMethod)discoveryMethod
{
    ...
    if (article) {
        ...
        if ([session.articleStore.sections count] > 0 && !session.articleStore.needsRefresh) {
            ...
            [self displayArticle:session.title leadImageName:@"Tunnel View, Yosemite Valley, Yosemite NP - Diliff.jpg"];
...
- (void)displayArticle:(MWKTitle *)title leadImageName:(NSString *)leadImageName
{

//TODO: remove the leadImageName parameter once article.image is available and just use the image
// from it instead.
...
//TODO: update this to get image name from article.image once brion adds it.
self.leadImageContainer.imageFileName = leadImageName;
